...
- Added a current V2 local matching run for 2026-07-02:
...
- `npm run validate:events -- external_events/2026-07-02.json`: passed, 70 events valid
...
- Remote `npm run verify` on Tencent Cloud after commit `419bf79`: passed, 173 passed / 0 failed; Gemini route contract validation deployed; PM2 `bci-tracker` online
- Remote `npm audit --audit-level=moderate` and `npm run verify` on Tencent Cloud after commit `f13373d`: passed, 0 vulnerabilities and 173 passed / 0 failed; dependency security upgrades deployed; PM2 `bci-tracker` online
- Remote `node scripts/test_allow_scripts_policy.js` and `npm run verify` on Tencent Cloud after commit `0d5ed15`: passed, install-script policy covered 2 packages and 173 passed / 0 failed; PM2 `bci-tracker` online. Remote npm is 10.8.2 and does not support `npm approve-scripts`, so use the project script there.
- Remote `npm run validate:events -- external_events/2026-07-02.json`, `node scripts/test_matching_context.js`, and `npm run verify` on Tencent Cloud after commit `54ab1b5`: passed, 70 events valid and 173 passed / 0 failed; 2026-07-02 V2 matching artifacts deployed; PM2 `bci-tracker` online.
- Remote `npm run validate:matching`, `npm run validate:v2-local`, and `npm run verify` on Tencent Cloud after commit `e5cc7d5`: passed, 2 matching reports valid and 173 passed / 0 failed; matching report shape validation deployed; PM2 `bci-tracker` online.
- Remote `node scripts/validate_api_docs.js` and `npm run verify` on Tencent Cloud after commit `87bf734`: passed, API docs covered 33 routes and 173 passed / 0 failed; requirements wording aligned to current implementation; PM2 `bci-tracker` online.
