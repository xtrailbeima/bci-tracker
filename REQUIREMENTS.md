...
- 抓取或导入后会按专题规则自动归集。
- 预设专题规则由系统维护；自定义专题支持创建时填写规则，并允许 owner/operator 在专题详情中编辑规则。
...
| POST | `/api/collections` | 创建专题 | `name`, `icon`, `rules[]` | owner/operator | 400 JSON error | 新建专题弹窗 |
| POST | `/api/collections/:id/add` | 添加文章到专题 | `articleId` | owner/operator | 500 JSON error | 文章归集弹窗 |
| PATCH | `/api/collections/:id/rules` | 更新自定义专题自动归集规则 | `rules[]` | owner/operator；预设专题不可编辑 | 400/404 JSON error | 专题详情规则编辑 |
...
- 所有 `/api/*` 业务接口默认要求登录，只有登录、登出、当前用户探测和必要静态资源例外。
...
- 已抽出 `services/ai_json.js`，供 DeepSeek 与 Gemini 共享 JSON 解析和修复逻辑。
- JSON 解析失败日志只记录 provider、错误消息和响应长度，不记录上游原文片段、API key 或用户敏感输入。
- DeepSeek 单篇分析、日报和周报在 JSON 解析失败时返回 `degraded: true`、稳定 `errorCode` 和人工确认提示；未配置、冷却中、上游 HTTP 错误和超时仍保持可区分响应。

验收标准：

- DeepSeek 日报/周报/单篇分析在无 key、冷却、上游失败下都有稳定响应结构。
- 冒烟测试覆盖未配置 key 分支和成功结构兼容分支；`scripts/test_deepseek_json.js` 覆盖公共 JSON 解析、日志脱敏和 degraded fallback。

### P1：测试独立性

- `npm test` 已通过 `scripts/run_smoke_with_server.js` 自行启动、等待并关闭本地测试服务；无需人工先启动服务。
- 现有冒烟测试已覆盖 URL 导入安全校验、专题创建/更新/删除、来源过滤、时间过滤、日期排序、评分一致性、DeepSeek 可用/未配置分支和 reader 权限边界。
...
- 前端已支持查看专题规则，新建自定义专题时可填写规则，owner/operator 可在专题详情编辑自定义专题规则。
- 后端已提供 `PATCH /api/collections/:id/rules`，规则只允许字符串关键词数组，最多 20 条、单条最多 40 字符，并限制危险字符。
- 预设专题默认不可删除且规则不可编辑；自定义专题可删除和更新规则。
...
- 决策：Gemini 保留为实验模块，不作为 DeepSeek fallback，不接入生产路由或前端入口。
- DeepSeek 继续作为当前生产 AI 分析主力，覆盖日报、周报和单篇文章分析。
- 若未来重新接入 Gemini，必须先新增需求切片，定义路由、缓存、冷却、前端入口、权限、成本边界和测试。

验收标准：

- 文档、路由和前端不再出现“模块存在但产品入口不明确”的状态。
- 自动验证应证明 `gemini.js` 没有被 `server.js` 或 `routes/api.js` 挂载，前端也没有 Gemini 入口。
