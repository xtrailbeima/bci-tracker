...
| GET | `/api/all` | 分页查询文章 | `q`, `category`, `source`, `sort`, `page`, `limit`, `from`, `to` | owner/operator/reader | 查询失败返回空列表结构 | 信息流 |
...
| GET | `/api/collections/:id` | 专题内容 | `page`, `limit` | owner/operator/reader | 500 JSON error | 专题详情 |
...
| GET | `/api/summary` | 旧版 Hunyuan 行业简报 | `force=1` | owner/operator，全局 10 分钟冷却 | 返回缓存、冷却提示或失败简报 | 兼容接口，当前主前端不作为 AI 分析主入口 |
...
当前 v5.1 已实现最小闭环：后端 session/cookie 登录、`owner/operator/reader` 三档权限、owner 用户管理、审计日志、前端登录门和按权限隐藏入口。后续仍可继续扩展邀请链接、密码重置、审计筛选和更细的数据脱敏。
