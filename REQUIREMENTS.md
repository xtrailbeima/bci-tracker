...
- Hunyuan 保留旧版 `/api/summary` 行业简报兼容接口。
...
|------|------|------|----------|-----------|----------|--------------|
| GET | `/api/all` | 分页查询文章 | `q`, `category`, `source`, `sort`, `page`, `limit`, `from`, `to` | 无 | 查询失败返回空列表结构 | 信息流 |
| GET | `/api/stats` | 获取文章统计 | 无 | 无 | 返回 0 统计 | 顶部统计栏 |
| GET | `/api/source-health` | 获取数据源抓取健康度 | 无 | 无 | 返回空数组 | 后续后台健康看板 |
| GET | `/api/trending` | 热门关键词 | `period=week/month/quarter/year` | 无 | 返回空数组 | 趋势关键词面板 |
| GET | `/api/sources` | 来源列表 | `category` | 无 | 返回空数组 | 来源过滤 |
| GET | `/api/collections` | 专题列表 | 无 | 无 | 500 JSON error | 专题追踪 |
| GET | `/api/collections/:id` | 专题内容 | `page`, `limit` | 无 | 500 JSON error | 专题详情 |
| POST | `/api/collections` | 创建专题 | `name`, `icon` | 无 | 400/500 JSON error | 新建专题弹窗 |
| POST | `/api/collections/:id/add` | 添加文章到专题 | `articleId` | 无 | 500 JSON error | 文章归集弹窗 |
| DELETE | `/api/collections/:id` | 删除非预设专题 | 无 | 无 | 500 JSON error | 专题追踪 |
| DELETE | `/api/collections/:collectionId/items/:articleId` | 从专题移除文章 | 无 | 无 | 500 JSON error | 后端已提供，前端暂未主路径使用 |
| POST | `/api/import` | 从 URL 导入文章 | `url` | 10 次/分钟 | 400 JSON error | 导入文章弹窗 |
| POST | `/api/subscribe` | 订阅每日简报 | `email`, `name` | 5 次/分钟 | 400/500 JSON error | 底部订阅表单 |
| POST | `/api/unsubscribe` | 退订每日简报 | `email` | 无 | 400/500 JSON error | 后端接口 |
| POST | `/api/briefing/send` | 手动触发邮件简报 | 无 | 无 | 500 JSON error | 运维/手动触发 |
| GET | `/api/summary` | 旧版 Hunyuan 行业简报 | `force=1` | 全局 10 分钟冷却 | 返回缓存、冷却提示或失败简报 | 兼容接口，当前主前端不作为 AI 分析主入口 |
| GET | `/api/summary/daily` | DeepSeek 每日速递 | `force=1` | 全局 2 分钟冷却 | 未配置 key 返回 503；失败返回错误摘要 | AI 分析：每日行业速递 |
| GET | `/api/summary/weekly` | DeepSeek 每周周报 | `force=1` | 全局 5 分钟冷却 | 未配置 key 返回 503；失败返回错误周报 | AI 分析：每周行业周报 |
| GET | `/api/analysis/:articleId` | DeepSeek 单篇文章分析 | 路径参数 `articleId` | 10 次/分钟 | 未配置 key 返回 503；错误返回 500 | 卡片 AI 分析按钮 |
| GET | `/api/pubmed` | PubMed 数据源 | `q`, `max` | 无 | 返回 PubMed demo 数据 | 后台抓取 |
| GET | `/api/arxiv` | arXiv 数据源 | `q`, `max` | 无 | 返回 arXiv demo 数据 | 后台抓取 |
| GET | `/api/journals` | 期刊 RSS 数据源 | 无 | 无 | 返回 Nature/Science demo 数据 | 后台抓取 |
| GET | `/api/news` | Google News 数据源 | 无 | 无 | 返回 news demo 数据 | 后台抓取 |
| GET | `/api/youtube` | YouTube 视频数据源 | 无 | 无 | 未配置 key 或错误时返回空数组 | 后台抓取 |
...
| `HUNYUAN_API_KEY` | 旧版 Hunyuan `/api/summary` | 兼容接口可选 | `/api/summary` 会回退尝试 `GEMINI_API_KEY` 作为 bearer key，但模型仍请求 Hunyuan |
...
- 将 `AGENTS.md` 的架构、API 清单和环境变量更新到 v5 真实状态。
- 统一前端版本文案，目前 header 仍显示 v4.0，footer/server 显示 v5.0。
- 明确 `bci-tracker/` 目录是否是历史副本、部署产物或误提交候选，并制定处理策略。
...
- 抽出 DeepSeek/Hunyuan/Gemini 共用 JSON 解析和修复工具，减少重复逻辑。
...
- 明确 Gemini 的产品角色：替代 Hunyuan、作为 fallback，还是保留为实验模块。
...
- `AGENTS.md` 仍偏 v4.2，缺少 YouTube、DeepSeek、URL 导入等 v5 内容。
- `public/index.html` header 显示 v4.0，footer 和 server 日志显示 v5.0。
- `gemini.js` 存在但没有在当前 `routes/api.js` 中作为主路由挂载。
- `bci-tracker/` 未跟踪目录看起来像旧版副本，本需求不把它作为当前实现依据。
